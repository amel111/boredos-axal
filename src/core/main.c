...
void kmain(void) {
    ...
    if (memmap_request.response != NULL) {
        ...
        // AXAL: Initialize buddy page frame allocator
        extern void pmm_init(struct limine_memmap_response *memmap);
        pmm_init(memmap_request.response);
        log_ok("PMM buddy allocator ready (AXAL)");

        smp_init_bsp();
        log_ok("SMP BSP initialized");

        // AXAL: Initialize per-CPU magazine caches (after SMP so we know CPU count)
        extern void pcpu_cache_init(void);
        pcpu_cache_init();
        log_ok("Per-CPU caches ready (AXAL)");
    } else {
    ...
    // AXAL: Initialize block cache for disk I/O acceleration
    extern void bcache_init(void);
    bcache_init();
    log_ok("Block cache ready (AXAL)");
