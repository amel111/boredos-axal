...
// AXAL: Set process priority and compute timeslice
void process_set_priority(process_t *proc, uint8_t priority) {
    if (!proc) return;
    if (priority > PROC_PRIO_IDLE) priority = PROC_PRIO_IDLE;
    proc->priority = priority;
    switch (priority) {
        case PROC_PRIO_REALTIME: proc->timeslice_max = TIMESLICE_REALTIME; break;
        case PROC_PRIO_HIGH:     proc->timeslice_max = TIMESLICE_HIGH; break;
        case PROC_PRIO_NORMAL:   proc->timeslice_max = TIMESLICE_NORMAL; break;
        case PROC_PRIO_LOW:      proc->timeslice_max = TIMESLICE_LOW; break;
        default:                 proc->timeslice_max = TIMESLICE_IDLE; break;
    }
    proc->timeslice = proc->timeslice_max;
}
...
void process_init(void) {
    ...
    // AXAL: Kernel/idle process gets lowest priority
    process_set_priority(kernel_proc, PROC_PRIO_IDLE);
    kernel_proc->vruntime = 0;
    kernel_proc->last_scheduled = 0;
...
process_t* process_create_elf(const char* filepath, const char* args_str, bool terminal_proc, int tty_id) {
    ...
    *(--stack_ptr) = 0;                // R15
    ...
    // AXAL: Set default priority based on process type
    if (terminal_proc) {
        process_set_priority(new_proc, PROC_PRIO_HIGH); // Interactive
    } else {
        process_set_priority(new_proc, PROC_PRIO_NORMAL);
    }
    new_proc->vruntime = 0;
    new_proc->last_scheduled = 0;
...
uint64_t process_schedule(uint64_t current_rsp) {
    ...
    // AXAL: Priority-based scheduler with timeslice
    // Decrement current process timeslice. Only switch if expired or higher priority waiting.
    extern uint32_t wm_get_ticks(void);
    uint32_t now = wm_get_ticks();

    // Check if current process still has timeslice remaining
    if (cur->timeslice > 0 && !cur->is_idle && cur->pid != 0xFFFFFFFF && !cur->kill_pending) {
        cur->timeslice--;
        
        // Check if a higher-priority process is waiting
        bool higher_waiting = false;
        process_t *scan = cur->next;
        while (scan != cur) {
            if (scan->cpu_affinity == my_cpu && scan->pid != 0xFFFFFFFF && !scan->kill_pending) {
                if (scan->priority < cur->priority) { // Lower number = higher priority
                    if (scan->sleep_until == 0 || scan->sleep_until <= now) {
                        higher_waiting = true;
                        break;
                    }
                }
    ...
    // Timeslice expired or higher priority waiting — find best candidate
    // Reset timeslice for current process
    if (cur->timeslice == 0) {
        cur->timeslice = cur->timeslice_max;
    }
    ...
    // Scan all processes for the highest priority runnable one on this CPU
    process_t *iter = cur->next;
    do {
        if (iter->cpu_affinity == my_cpu && iter->pid != 0xFFFFFFFF && !iter->kill_pending) {
            if (iter->sleep_until == 0 || iter->sleep_until <= now) {
    ...
    // If no better candidate, stay on current
    if (!best || (best == cur)) {
        spinlock_release_irqrestore(&runqueue_lock, rflags);
        if (cleanup_stack) kfree(cleanup_stack);
        if (cleanup_pml4) {
