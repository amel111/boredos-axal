...
// AXAL: TLB shootdown IPI — invalidates TLB on all other CPUs
void lapic_send_tlb_shootdown(void) {
    if (!lapic_base) return;
    
    // Send TLB shootdown IPI to all excluding self
    uint32_t icr_low = IPI_TLB_SHOOTDOWN_VECTOR | (0b11 << 18) | (1 << 14);
    lapic_base[LAPIC_ICR_LOW] = icr_low;
}
