...
// AXAL: Increased from 16 to 64 for real workloads
...
// AXAL: Process priority levels for scheduler
#define PROC_PRIO_REALTIME  0   // Highest — never preempted by lower
#define PROC_PRIO_HIGH      1   // Interactive (GUI, input handling)
#define PROC_PRIO_NORMAL    2   // Default for user processes
#define PROC_PRIO_LOW       3   // Background tasks
#define PROC_PRIO_IDLE      4   // Only runs when nothing else can

// AXAL: Time slice in ticks per priority level
...
typedef struct process {
    ...
    // AXAL: Scheduler priority and time accounting
    uint8_t priority;           // PROC_PRIO_*
    uint32_t timeslice;         // Remaining ticks before preemption
    uint32_t timeslice_max;     // Reset value based on priority
    uint64_t vruntime;          // Virtual runtime for fair scheduling
    uint64_t last_scheduled;    // Tick when last scheduled (for aging)
