...
// AXAL: Optimized memory operations using x86 rep string instructions.
// Modern CPUs with ERMS (Enhanced REP MOVSB/STOSB) execute these at near-memcpy speeds
// with hardware-optimized microcode, eliminating the byte-at-a-time loop overhead.
...
static void mem_memmove(void *dest, const void *src, size_t len) {
    if (!len) return;
    if (dest < src || (uintptr_t)dest >= (uintptr_t)src + len) {
        // Forward copy is safe — no overlap or dest is before src
        asm volatile(
            "rep movsb"
            : "+D"(dest), "+S"(src), "+c"(len)
            :
        ...
        const uint8_t *s = (const uint8_t *)src + len - 1;
        asm volatile(
            "std\n\t"
            "rep movsb\n\t"
            "cld"
