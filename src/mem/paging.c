...
void paging_map_page(uint64_t pml4_phys, uint64_t virtual_addr, uint64_t physical_addr, uint64_t flags) {
    ...
    // Flush TLB for this address on this CPU
    asm volatile("invlpg (%0)" : : "r"(virtual_addr) : "memory");
    
    // AXAL: Send TLB shootdown IPI to other CPUs for kernel-space mappings
    // User-space mappings are per-process and handled by CR3 switch on context switch
    if (virtual_addr >= 0xFFFF800000000000ULL) {
        extern void lapic_send_tlb_shootdown(void);
        extern uint32_t smp_cpu_count(void);
        if (smp_cpu_count() > 1) {
            lapic_send_tlb_shootdown();
        }
    }
}
